...
void Game::InitialiseGame()
{
	...
	m_Missile.Initialise(Vector2(m_PlayerShip.GetPosition().x + (m_PlayerShip.GetSize().x / 2), m_PlayerShip.GetPosition().y - 2), Vector2(0, -MISSILE_SPEED));
...
{
	m_Missile.SetPosition(m_PlayerShip.GetPosition().x + (m_PlayerShip.GetSize().x / 2), m_PlayerShip.GetPosition().y - 2);
	m_Missile.SetActive();
}
